"""Focused deterministic and governance tests for Phase 4E."""
...
from pipeline.phase4d_report import (
    _historical_before_compatibility_matches as phase4d_historical_before_matches,
    _isolation_snapshot as phase4d_isolation_snapshot,
)
from pipeline.phase4e_report import _deprecated_allergen_checksum_sql, _isolation_snapshot


class Phase4ETests(unittest.TestCase):
    @classmethod
    ...
    def test_scoped_aliases_do_not_leak_into_phase4e_scopes(self) -> None:
    ...
    def test_parent_child_safety_prevents_unsupported_children(self) -> None:
        ...
        safe_matches = match_ingredients(
            [
                self.evidence("Sugar", country="DE", category="Snacks"),
                self.evidence(
                    "Salt",
                    country="DE",
                    category="Snacks",
        ...
        self.assertEqual(len(linkable_matches(safe_matches, self.registry)), 2)
    ...
    def test_generated_outputs_are_idempotent_ordered_and_platform_stable(self) -> None:
        second_outputs, second_stats = build_outputs(PHASE4E_PATH)
        ...
        self.assertTrue(all("-- Phase: 4E" in content for content in self.outputs.values()))
        self.assertTrue(
            all(
    ...
    def test_phase4b_phase4c_and_phase4d_historical_results_are_unchanged(self) -> None:
        ...
        _, phase4d_stats = build_outputs(PHASE4D_PATH)
        self.assertEqual(phase4b_stats["selected_products"], 941)
        self.assertEqual(phase4b_stats["linked_ingredient_rows"], 9988)
        self.assertEqual(phase4d_stats["selected_products"], 805)
        self.assertEqual(phase4d_stats["linked_ingredient_rows"], 11051)
        self.assertEqual(
            self.report["phase4d_report_checksum"],
    ...
    def test_phase4d_compatibility_masks_only_legacy_identity_checksum(self) -> None:
        expected = json.dumps(
            {
                "isolation": {"non_target_product_checksum": "legacy"},
                "snapshot": {"ingredient_links": 33971},
            }
        )
        deterministic_variant = json.dumps(
            {
                "isolation": {"non_target_product_checksum": "deterministic"},
                "snapshot": {"ingredient_links": 33971},
            }
        )
        data_regression = deterministic_variant.replace("33971", "33972")
        self.assertTrue(phase4d_historical_before_matches(expected, deterministic_variant))
        self.assertFalse(phase4d_historical_before_matches(expected, data_regression))
    ...
    def test_non_target_identity_checksum_has_a_total_cross_platform_order(self) -> None:
        for snapshot in (_isolation_snapshot, phase4d_isolation_snapshot):
            source = inspect.getsource(snapshot)
            self.assertIn("COALESCE(p.ean,'') COLLATE", source)
            self.assertIn("p.product_name COLLATE", source)
            self.assertIn("p.product_id", source)

    def test_phase4e_post_processing_refreshes_analytical_views_last(self) -> None:
        post_sql = (PROJECT_ROOT / "db" / "ci_post_phase4e.sql").read_text(encoding="utf-8").rstrip()
