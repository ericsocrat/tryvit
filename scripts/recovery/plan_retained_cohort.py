...
def retained():
    ...
    for member in receipt["members"]:
        record = json.loads(cohort._artifact_path(RUN, member["observation_file"], must_exist=True).read_text(encoding="utf-8"))
...
def prepare():
    ...
    request = [{"product_id": m["product_id"], "country": m["country"], "ean": m["ean"],
                "source_brand": o["identity"]["brand"], "source_name": o["identity"]["product_name"]} for m, o in records]
    query = f"""WITH cohort AS (SELECT * FROM jsonb_to_recordset({_sql_text(json.dumps(request, ensure_ascii=False))}::jsonb)
    ...
    SELECT jsonb_build_object('checked_at',now(),'total_products',(SELECT count(*) FROM public.products),
      ...
      'batch_count',(SELECT count(*) FROM public.ingestion_batches),'observation_count',(SELECT count(*) FROM public.product_source_observations),
      'members',(SELECT jsonb_agg(jsonb_build_object('product_id',c.product_id,
        'product',(SELECT jsonb_build_object('product_id',p.product_id,'country',p.country,'ean',p.ean,'brand',p.brand,
          'product_name',p.product_name,'category',p.category,'is_deprecated',p.is_deprecated,'source_type',p.source_type,
          'source_url',p.source_url,'source_ean',p.source_ean,'last_fetched_at',p.last_fetched_at,'off_revision',p.off_revision,'updated_at',p.updated_at)
          FROM public.products p WHERE p.product_id=c.product_id),
        'market_ean_ids',(SELECT COALESCE(jsonb_agg(p.product_id ORDER BY p.product_id),'[]'::jsonb) FROM public.products p WHERE p.country=c.country AND p.ean=c.ean),
        'source_name_ids',(SELECT COALESCE(jsonb_agg(p.product_id ORDER BY p.product_id),'[]'::jsonb) FROM public.products p WHERE p.country=c.country
          AND lower(btrim(p.brand))=lower(btrim(c.source_brand)) AND lower(btrim(p.product_name))=lower(btrim(c.source_name)) AND p.is_deprecated IS NOT TRUE),
        'other_market_ids',(SELECT COALESCE(jsonb_agg(p.product_id ORDER BY p.product_id),'[]'::jsonb) FROM public.products p WHERE p.country<>c.country AND p.ean=c.ean),
        'provenance',(SELECT COALESCE(jsonb_agg(jsonb_build_object('field',p.field_name,'source',p.source_type,
          'has_verification',p.verified_by IS NOT NULL OR p.verified_at IS NOT NULL)),'[]'::jsonb) FROM public.product_field_provenance p WHERE p.product_id=c.product_id),
        'provenance_sha256',(SELECT encode(sha256(convert_to(COALESCE(string_agg(to_jsonb(p)::text,E'\\n' ORDER BY p.field_name),''),'UTF8')),'hex')
          FROM public.product_field_provenance p WHERE p.product_id=c.product_id),
        'source_mapping',(SELECT jsonb_build_object('product_id',s.product_id,'selected_observation_id',s.selected_observation_id,
          'retrieved_at',o.retrieved_at,'source_updated_at',o.source_updated_at,'source_revision',o.source_revision,'payload_hash',o.payload_hash)
          FROM public.product_source_records s LEFT JOIN public.product_source_observations o ON o.id=s.selected_observation_id
        ...
        'nutrition',(SELECT jsonb_build_object('calories_100g',n.calories::text,'fat_100g',n.total_fat_g::text,
          'saturated_fat_100g',n.saturated_fat_g::text,'trans_fat_100g',n.trans_fat_g::text,'carbs_100g',n.carbs_g::text,
          'sugars_100g',n.sugars_g::text,'fiber_100g',n.fibre_g::text,'protein_100g',n.protein_g::text,'salt_100g',n.salt_g::text)
          FROM public.nutrition_facts n WHERE n.product_id=c.product_id)) ORDER BY c.product_id) FROM cohort c)) AS state;"""
    PLAN.mkdir(parents=True, exist_ok=True)
    (PLAN / "production-query.sql").write_text(query, encoding="utf-8")
    print(json.dumps({"retained": proof, "query_artifact": str(PLAN / "production-query.sql"), "network_refetch": False}))
...
def report():
    ...
    for member, observation in records:
        ...
        if product:
            ...
            if any((product[k] or "").strip().lower() != (observation["identity"][k] or "").strip().lower() for k in ("brand", "product_name")):
                reasons.append("identity_text_change_requires_review")
            if product["last_fetched_at"] and datetime.fromisoformat(product["last_fetched_at"].replace("Z", "+00:00")) > datetime.fromisoformat(observation["retrieved_at"].replace("Z", "+00:00")):
                reasons.append("existing_projection_fetched_later")
            if product["off_revision"] and observation["source_revision"] and product["off_revision"] > observation["source_revision"]:
        ...
        for field, value in observation["extracted_fields"].items():
            if field in (db["nutrition"] or {}):
                after = value.get("value") if value.get("state") == "recorded" and value.get("qualifier") == "eq" else None
                changes.append({"field": field, "before": db["nutrition"][field], "projected_after": after,
                                "basis": value.get("basis"), "qualifier": value.get("qualifier"), "state": value.get("state")})
        rows.append({"product_id": member["product_id"], "country": member["country"], "category": member["category"], "ean": member["ean"],
                     "eligible_unchanged_retained_record": not reasons, "hold_reasons": reasons, "retained_category": category.get("value"),
                     "current_policy_category": resolved, "retained_policy": category.get("policy_version"),
                     "retrieved_at": observation["retrieved_at"], "source_updated_at": observation["source_updated_at"],
                     "payload_hash": observation["payload_hash"], "record_sha256": member["observation_sha256"], "before_product": product,
                     "source_mapping": db["source_mapping"], "other_market_ids": db["other_market_ids"],
                     "market_ean_ids": db["market_ean_ids"], "source_name_ids": db["source_name_ids"],
                     "provenance": db["provenance"], "provenance_sha256": db["provenance_sha256"], "nutrient_projection": changes})
    eligible = sorted((r for r in rows if r["eligible_unchanged_retained_record"]), key=lambda r: r["product_id"])
    result = {"method": "retained-observation-import-plan-no-refetch-no-apply", "created_at": datetime.now(UTC).isoformat(),
              "production_checked_at": state["checked_at"], "cohort": proof, "category_policy": CATEGORY_POLICY_VERSION,
              "production_counts": {k: state[k] for k in ("total_products", "active_products", "batch_count", "observation_count")},
              "eligible_count": len(eligible), "smallest_candidate_id": eligible[0]["product_id"] if eligible else None,
              "hold_counts": dict(Counter(reason for r in rows for reason in r["hold_reasons"])), "members": rows}
    path = PLAN / "plan.json"
    path.write_text(json.dumps(result, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    print(json.dumps({k: result[k] for k in ("cohort", "category_policy", "production_counts", "eligible_count", "smallest_candidate_id", "hold_counts")}, ensure_ascii=False))
...
def pilot():
    ...
    candidates = sorted((r for r in plan["members"] if r["eligible_unchanged_retained_record"]), key=lambda r: r["product_id"])
    if not candidates:
    ...
    batch = {"source_key": "off_api", "country": member["country"], "extractor_version": EXTRACTOR_VERSION,
             "idempotency_key": f"{member['country']}:{digest}", "scope": {"category": member["category"], "kind": "partial_upsert"}}
    ...
    columns = {"calories_100g": "calories", "fat_100g": "total_fat_g", "saturated_fat_100g": "saturated_fat_g",
               "trans_fat_100g": "trans_fat_g", "carbs_100g": "carbs_g", "sugars_100g": "sugars_g",
               "fiber_100g": "fibre_g", "protein_100g": "protein_g", "salt_100g": "salt_g"}
    nutrient_expr = "jsonb_build_object(" + ",".join(_sql_text(k) + ",n." + v + "::text" for k, v in columns.items()) + ")"
    tag = "$retained_pilot$"
    while tag in record_sql or tag in batch_sql:
        tag = tag[:-1] + "x$"
    sql = f"""-- DRY PLAN ONLY: requires a fresh 19-table recovery proof and isolated import/reversal rehearsal.
...
BEGIN
  ...
  IF result->>'status' IS DISTINCT FROM 'accepted' OR (result->>'product_id')::bigint IS DISTINCT FROM {member['product_id']} THEN
    RAISE EXCEPTION 'pilot_did_not_accept_exact_existing_product';
  END IF;
  IF NOT EXISTS(SELECT 1 FROM public.product_source_records s JOIN public.product_source_observations o ON o.id=s.selected_observation_id
    WHERE s.source_key='off_api' AND s.country={_sql_text(member['country'])} AND s.external_id={_sql_text(member['ean'])}
...
"""
    ...
    print(json.dumps({"sql_artifact": str(path), "sha256": hashlib.sha256(sql.encode()).hexdigest(),
                      "product_id": member["product_id"], "country": member["country"], "record_unchanged": True,
                      "executed": False, "recovery_19_tables": "REQUIRED_NOT_YET_PROVEN"}))
