...
SELECT '33. canonical search envelope keys' AS check_name,CASE WHEN ((SELECT pg_temp.qa_keys(b,ARRAY['api_version','policy_version','query','country','language','total','page','pages','page_size','filters_applied','preferences_applied','results','excluded_summary','excluded_exact_match']) FROM qa_contract_find)) IS TRUE THEN 0 ELSE 1 END AS violations;
