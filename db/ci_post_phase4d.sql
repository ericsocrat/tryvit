-- Phase 4D scoped post-enrichment processing.
-- Applies the existing deterministic allergen and concern rules only to the
-- four approved Phase 4D category-country scopes. Non-target products are
...
WHERE pi.ingredient_id = ir.ingredient_id
  ...
  AND (
    (p.category = 'Sweets' AND p.country = 'PL')
    OR (p.category = 'Dairy' AND p.country = 'PL')
    OR (p.category = 'Frozen & Prepared' AND p.country = 'DE')
    OR (p.category = 'Meat' AND p.country = 'DE')
  );
...
SET concern_tier = GREATEST(COALESCE(ir.concern_tier, 0), c.target_concern_tier),
    ...
    concern_reason = COALESCE(
      ...
      'Phase 4D scoped deterministic additive/E-code classifier'
    )
FROM classified c
...
-- Preserve the repository's top-level position invariant, scoped to Phase 4D.
DO $$
DECLARE
  row_to_fix RECORD;
BEGIN
...
CALL score_category('Sweets', p_country := 'PL');
CALL score_category('Dairy', p_country := 'PL');
