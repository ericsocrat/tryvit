...
**Location:** `supabase/migrations/` — managed by Supabase CLI. Enumerate the current set and application state with `supabase migration list --local`.
...
| DB QA (786 checks; 778 blocking) | Raw SQL (zero rows = pass)          | `db/qa/QA__*.sql` (50 suites)                | `.\RUN_QA.ps1`                       |
...
- Run `.\RUN_QA.ps1` to verify all 786 checks pass (778 blocking) + `.\RUN_NEGATIVE_TESTS.ps1` for 20 injection tests.
...
**Run:** `.\RUN_QA.ps1` — expects **786/786 checks passing** (778 blocking, + EAN validation).
...
- [ ] `.\RUN_QA.ps1` — all 786 checks pass (778 blocking)
...
.\RUN_QA.ps1                      → 786/786 checks pass (778 blocking, 0 failures)
.\RUN_NEGATIVE_TESTS.ps1          → 20/20 caught
