...
| Authenticated quality routes                                                                                                                           | Local-authenticated                           | Successful `auth-setup` dependency → load owned storage state → create context/install guard → create page → navigate/audit/capture → close context/assert → guarded global fixture deletion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Standard hosted CI is blocked earlier because it does not provision the emulator.                                                                                                                                                                                                                            |
...
| Quality Gate public audits                                                                                                                             | Backend-independent public                    | Checkout/install → select smoke/full → blocking public preflight → public mobile launcher → public desktop launcher → public Lighthouse launcher → mandatory public final assert → conditional artifact upload                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Public results exclude routes that require authentication.                                                                                                                                                                                                                                                   |
| Quality Gate authenticated audits                                                                                                                      | Local-authenticated                           | Blocking local-authenticated preflight → authenticated mobile launcher → authenticated desktop launcher → authenticated Lighthouse gate → conditional final assert/artifact upload                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | The workflow does not provision a local emulator, so a standard hosted runner is intentionally blocked at preflight. Even with an emulator, authenticated Lighthouse is intentionally blocked until a dedicated guarded local fixture exists. This is a reported blocker, not a pass or a public substitute. |
...
| Nightly authenticated browser suite                                                                                                                    | Local-authenticated                           | Blocking local-authenticated preflight → authenticated/functional launcher → local cleanup → mandatory final assert → conditional report upload                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | The workflow does not provision a local emulator, so a standard hosted runner is intentionally blocked at preflight. Hosted staging/production is never substituted.                                                                                                                                         |
| Lighthouse CI and `npm run visual-safety:public-lighthouse`                                                                                            | Backend-independent public                    | Sanitize environment/load public contract → require supported Node environment-proxy behavior → prove port/`.next` ownership → start owned proxy → clean build/scan/provenance through the proxy → start owned Next server through the proxy → create temporary Lighthouse config → run mobile/desktop audit → final proxy/provenance assert → stop owned resources → remove temporary config                                                                                                                                                                                                                                                                                                                                                                                                            | Public only. Authenticated mode is rejected until a dedicated guarded local fixture exists.                                                                                                                                                                                                                  |
...
Phase 5A.0c. All other unexpected non-loopback CONNECT attempts fail closed.
...
At Phase 5A.0a completion, the scheduled Quality Gate and Nightly workflows do
not provision a Supabase emulator and therefore cannot claim authenticated
coverage on their standard hosted runners. Their authenticated preflights are
deliberately blocking. Quality Gate's authenticated Lighthouse stage is also
deliberately blocked by the launcher pending a dedicated guarded local fixture.
Public quality and public Lighthouse results are valid only for their public
route scope and are not authenticated equivalents.
