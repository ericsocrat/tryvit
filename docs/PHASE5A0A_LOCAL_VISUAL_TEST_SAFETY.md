...
| Nightly authenticated browser suite                                                                                                                    | Local-authenticated                           | Install pinned CLI → start the same reduced ephemeral stack → guarded readiness and fixture seed → authenticated/functional launcher → mandatory final assert → guarded fixture teardown → unconditional no-backup runtime stop → conditional report upload                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | The fixture supplies the existing functional contract without Phase 4 enrichment or a full catalog. The separate schedule-only `Production Data Integrity Audit` remains outside browser evidence.                                                                        |
| Phase 5A.0d Lighthouse workflow and guarded launchers                                                                                                  | Public plus local-authenticated               | Attest literal source head → sanitize environment → run the guarded public five-run Mobile/Desktop matrix → start the reduced local runtime → guarded readiness and fixture seed → run the guarded authenticated five-run Mobile/Desktop matrix → redact and aggregate evidence → mandatory fixture/runtime cleanup → upload only compact credential-free evidence                                                                                                                                                                                                                                                                                                                                                                                                                                       | This separate blocking workflow owns the authoritative performance contract. Source configuration files and thresholds remain unchanged; Quality Gate does not duplicate or claim Lighthouse coverage.                                                                   |
...
seed is intentionally empty. Postgres, Kong/API, GoTrue, PostgREST, and local
Realtime are kept. Phase 5A.0d requires Realtime for the existing feature-flag
subscription and derives its exact loopback `ws:` CSP source from the checked-in
local HTTP origin. Storage, image transformation, Studio, Mailpit, Edge
Functions, analytics, metadata, and the pooler remain excluded.
...
The separate schedule-only `Production Data Integrity Audit` remains independently
governed. It is not dispatched for Phase 5A.0a closure verification because it
accesses the fixed production data environment and is not evidence for the browser
runtime.
...
quality results remain valid only for their public route scope and are never
reported as authenticated equivalents. Phase 5A.0d now owns Lighthouse in a
separate guarded workflow covering public and local-authenticated Mobile and
Desktop routes with five-run distributions. Quality Gate neither runs nor
claims Lighthouse coverage.
