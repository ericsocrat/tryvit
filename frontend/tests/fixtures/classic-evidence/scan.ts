...
const base = { api_version: z.literal("2"), policy_version: z.literal(EVIDENCE_POLICY_VERSION), scan_country: z.enum(["PL", "DE"]).nullable() };
export const EvidenceScanSchema = z.discriminatedUnion("found", [
  z.object({ ...base, found: z.literal(true), product: ProductReadModelSchema }),
  z.object({ ...base, found: z.literal(false), ean: z.string().regex(/^(?:[0-9]{8}|[0-9]{13})$/), has_pending_submission: z.boolean() }),
]).transform((data) => data.found ? {
...
export const ScanHistoryEvidenceSchema = z.object({
  ...
  total: z.number().int().nonnegative(), page: z.number().int().positive(), pages: z.number().int().positive(),
  page_size: z.number().int().min(1).max(50), filter: z.enum(["all", "found", "not_found"]),
  scans: z.array(z.object({
    scan_id: z.uuid(), ean: z.string(), found: z.boolean(), scanned_at: z.iso.datetime({ offset: true }),
    product_id: z.number().int().positive().nullable(), product_name: z.string().nullable(),
    brand: z.string().nullable(), category: z.string().nullable(), submission_status: z.string().nullable(),
  })).max(50),
}).refine((data) => data.scans.length <= data.page_size && data.scans.length <= data.total, "Invalid history count");
