...
function FindWorkspace() {
  ...
  return <AppPage>
    <AppPageHeader eyebrow={t("nav.find")} title={t("findUi.title")} description={t("findUi.description")} compactOnMobile />
    ...
    {preferences.isPending ? <p role="status">{t("findUi.loadingPreferences")}</p> : preferences.error || !validContext ? <section className={styles.notice} role="alert"><h2>{t("findUi.preferencesUnavailable")}</h2><p>{t("findUi.preferencesRequired")}</p><Button onClick={() => void preferences.refetch()}>{t("common.retry")}</Button><Link href="/app/settings" className={styles.textLink}>{t("nav.settings")}</Link></section> : <>
      <details className={styles.contextDisclosure}>
        <summary><span className={styles.contextSummaryCopy}><span>{t("findUi.marketContext", { country })}</span><span>{t("findUi.contextTitle")}</span></span><ChevronDown className={styles.disclosureIcon} size={18} aria-hidden="true" /></summary>
        <div className={styles.context}><p>{t("findUi.preferenceContext")} <Link href="/app/settings/nutrition">{t("findUi.reviewPreferences")}</Link></p>
