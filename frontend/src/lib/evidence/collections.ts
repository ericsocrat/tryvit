...
export const SavedListSchema = z.object({
  ...
  share_enabled: z.boolean(), share_token: z.nullable(z.string()), total_count: z.int().check(z.nonnegative()),
  limit: z.int().check(z.minimum(1)).check(z.maximum(100)), offset: z.int().check(z.nonnegative()),
  items: z.array(z.object({ item_id: z.uuid(), product_id: z.int().check(z.positive()), position: z.int(), notes: z.nullable(z.string()), added_at: z.string(), product: z.nullable(ProductReadModelSchema) }).check(z.refine((item) => item.product === null || item.product.product_id === item.product_id, "Saved product identity mismatch"))).check(z.maxLength(100)),
...
export const WatchedProductsSchema = z.object({
  api_version: z.literal("2"), policy_version: z.literal(EVIDENCE_POLICY_VERSION), total: z.int().check(z.nonnegative()),
  page: z.int().check(z.positive()), page_size: z.int().check(z.minimum(1)).check(z.maximum(100)), total_pages: z.int().check(z.positive()),
  items: z.array(z.object({ watch_id: z.int().check(z.positive()), product_id: z.int().check(z.positive()), watched_since: z.string(), product: z.nullable(ProductReadModelSchema) }).check(z.refine((item) => item.product === null || item.product.product_id === item.product_id, "Watched product identity mismatch"))).check(z.maxLength(100)),
